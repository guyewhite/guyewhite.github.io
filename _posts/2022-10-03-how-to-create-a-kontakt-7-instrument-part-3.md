...
### Step One - Buttons and More Buttons
  ...
  * Second, let's add three other buttons as well. Modify your code as follows:
    ...
    on init
      ...
      {declare another button called $option3 that is tied to a button. The intial value is set to off}
    ...
    ```
  * Let's discect this code a bit. Notice that a new button is added with the following code:
...
### Step Three - The State of Things
  ...
  * Now, all of all of our buttons are in place for our first Kontakt instrument. We are almost to the point where our script can start doing things musically interesting. To prepare for musical magic, we have some final edits to complete.
  * First, let's modify the initial value of `$run`. Up until this point, by default it has been set to the `on` position. Modify you code in `on init` as follows:
  ...
  * Fourth, let's make it so the `run` button is set to `off` every time the `A`, `B` or `C` option is changed. Ineach of our `on ui_control ($option)` callbacks, we will add another line that turns off `run`. For example, for `on ui_control ($option1)` we will modify the code as follows:
  ...
  * Finally, we will modify the `on ui_control($run)` callback as follows:
    ...
    ```
    Notice that when the the `run` button is pressed, it places a message in the console that echos the state that is running, whether `A`, `B`, or `C`. Otherwise, the console messages `Ready to run`.
...
### Step Four - Your First Function
  ...
  * Functions are callbacks of your own design. Functions can be useful in executing blocks of code at will -- especially those that are repeated throughout your code. Notice how a specific block of code tends to repeat over and over again:
    ...
    message("Ready to run")
    ```
    This code appears no less than four places currently.
  ...
  * Further, in the three places in the `on ui_control` for `$option1`, `$option2`, and `$option3`, replace the two lines that reset the `$run` variable and messages `Ready to run` to `call resetRun()`. Sadly, we cannot utilize such functions in `on init` -- which makes me sad as a programmer. In the end, your final code should appear as follows:
    ...
    ```
    Notice how the function called `resetRun` must appear above those callbacks that utilize this function. Through the utilization of this function, we are *abstracting away* or simplifying, our code by taking a task that took six lines of code and reduced it down to a single line of code. Abstraction of this nature will become of particular importance as you develop as a Kontakt Script Language developer.
...
The tutorial to follow will allow you to *finally* produce something musically intresting.
